...
function GamePage() {
    ...
    return (
        ...
        <h1 className='mb-2 font-bold text-center text-4xl text-black-200 backdrop-blur-sm'>Play!</h1>
        <img className = 'm-auto rounded-lg border-2 border-indigo-600' src = {image} alt ={miku} width={400} length = {400} ></img>
        <p className='mb-2 font-bold text-black-500 text-center text-3xl backdrop-blur-sm'>Please enter your guess here (or type "skip"!): {correctAnswer}</p>
        <input className='m-auto display-block border-2 border-indigo-600 rounded-lg' type="text" onChange={handleChange} value ={userGuess}/>
        <h1 className='mb-2 font-bold text-black-700 text-center text-3xl backdrop-blur-sm'>Current Score is: {playerScore}</h1>
        <h1>{cleanGuess}</h1>
        <h1 className='mb-2 font-bold text-black-700 text-center text-3xl backdrop-blur-sm'>Time Remaining:</h1>
        <h1 className='mb-2 font-bold text-black-700 text-center text-3xl backdrop-blur-sm'>{timer}</h1>
        </div>
        </>
    );
    ...
    <h1 className='font-bold text-3xl backdrop-blur-sm'>Thanks for playing!</h1>
    <h1 className='font-bold text-3xl backdrop-blur-sm'>Your final score is {playerScore}!</h1>
    <button className ='h-screen w-screen max-w-xs max-h-1 flex items-center justify-center'onClick={playAgain}>Play Again!</button>
    </div>
    </>
    )
}
